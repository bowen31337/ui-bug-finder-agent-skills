...
2. Read the example specs for reference:
   ```
   ui-bug-scanner/assets/example-specs.json
   ```

3. Create a custom rules JSON file based on their requirements and save it to `ui-bug-scanner/assets/custom-specs.json`.
...
Save the rules to `ui-bug-scanner/assets/custom-specs.json` and provide the command to run the scan with these rules:

```bash
cd ui-bug-scanner
npx ts-node scripts/scanner.ts --url <URL> --specs ./assets/custom-specs.json --output ./reports
```
